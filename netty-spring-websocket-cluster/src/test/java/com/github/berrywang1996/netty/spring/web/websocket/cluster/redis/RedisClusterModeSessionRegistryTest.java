...
class RedisClusterModeSessionRegistryTest {
    ...
    void registerIssuesHmsetAndSadd() {
        ...
        // Session hash must carry the owning nodeId field.
        verify(async).hmset(eq("netty:session:/ws/a:sid-1"),
                argThat((Map<String, String> m) -> "node-A".equals(m.get("nodeId"))));
        // Node-set membership added under the node key, member "uri|sessionId".
        verify(async).sadd("netty:node:node-A:sessions", "/ws/a|sid-1");
    ...
    void deregisterIsNonAtomic_hgetThenDelAndSrem_noEval() {
        ...
        verify(async).hget("netty:session:/ws/a:sid-1", "nodeId");
        // ...then the session hash deleted and the node-set member removed, as SEPARATE commands.
        verify(async).del("netty:session:/ws/a:sid-1");
        verify(async).srem("netty:node:node-A:sessions", "/ws/a|sid-1");
    ...
    void deregisterMissingSession_hgetOnly_noDelNoSrem() {
        ...
        verify(async).hget("netty:session:/ws/a:sid-x", "nodeId");
