...
- **当前推荐版本：`1.8.0`**（WebSocket 集群支持：Redis Pub/Sub 跨节点 + 5 层 SPI + 289 测试。单机模式生产级；集群模式面向 ≤~10 节点 + 专用加密 Redis，详见 `docs/release-notes-1.8.0.md` 与 `cluster-design.md §安全模型`）。
...
- 289 个测试（含 6 SPI 隔离 + 7 配置/行为 + 9 Redis 集成 + 3 auto-config 装配；另有 4 个性能基准为手动 harness，不计入套件），11 模块全绿。详见 `docs/release-notes-1.8.0.md`。
...
- ✅ 全量 `mvn test` 通过（289 测试 / 11 模块）；✅ 集群路径有真实 Redis 集成测试 + auto-config 装配测试（⏳ Testcontainers 化推迟）。
