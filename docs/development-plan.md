...
- **`1.10.0` GA——2026-06-21 已发布到 Maven Central（`v1.10.0`，FF 合并到 master）（「IM 平台基础」线，Boot 2.7 栈，2.0.0 Boot 3.x 仍独立）**：RC1 `ClusterRoomRegistry`（**per-room 节点定向路由** —— 见下方 ⚠️ 设计修正）+ RC2 消息历史/离线队列 + RC3 多设备 presence + RC4 node-to-node mesh。定位驱动：1.9.0 GA 后做了竞品深挖 + 定位研究（见 `docs/superpowers/notes/`），结论是 IM/实时通信原语 + 突破节点天花板是最高背书方向。
...
## `1.10.0` IM 平台基础线（GA——2026-06-21 已发布到 Maven Central）
...
| RC4 | node-to-node mesh（直连 Netty TCP；把 Redis 移出消息热路径）| **真正的节点天花板突破** + 多月工程 | 🔧 **RC4a 已 cut `v1.10.0-RC4a`**（mesh 传输地基：直连 unicast + 朴素广播,**无扇出削减**；设计审查 M1 BLOCKER + M2–M5 折叠,实现审查 MF1 成员∩心跳/MF2 连接超时 2 MAJOR 折叠；601 测试/11 模块；扇出削减=RC4b；未 push/deploy）→ 🔧 **RC4b 已 cut `v1.10.0-RC4b`**（**按会话**兴趣路由的扇出削减——节点对 URI 感兴趣 ⟺ 有活跃会话,同构集群上对分区受众也真实削减；3 轮对抗式设计审查 12+3+1 折叠 + 实现审查 7 MINOR 折叠；620 测试/11 模块；诚实声明随机 LB 人口饱和前提；未 push/deploy）→ 🔧 **RC4c 已 cut `v1.10.0-RC4c`**（mesh 热路径健壮性：BL5 对端地址快照把 Redis 移出广播热路径（unicast 直查回退+预热,保留 ~0 不可达窗口）、仅热路径重连退避（tick 裸拨号保持恢复探针）、WRITER_IDLE 空闲回收、BL4 传输状态对账、BL2 传输冲突快速失败守卫；2 轮设计审查 3 MAJOR+6 nit + 第二轮 1 MAJOR 折叠 + 实现审查 6 MINOR 折叠；629 测试/11 模块；未 push/deploy）→ 🔧 **RC4d 已 cut `v1.10.0-RC4d`**（mesh 可观测性：9 个 `netty.cluster.mesh.*` 指标——frames.received/sent、send.failures、send.dropped_backpressure、idle.reaps、reconnect.backoff_skips 六个计数器 + fanout.target_nodes（扇出削减观测点，复用 room 采样器）、connections.active、peers.known 三个 gauge；**BL6：binder 直接读 broker 自己的 `ClusterRuntimeStats`**（无共享/无 swap——broker 在 sender bean 前自启动）；顺带把 `backoff_skips` 与 `send.failures` 改为不相交计数；`instanceof MeshBroker` 门控（Redis 路径零 mesh 指标）；4 镜头对抗式实现审查 0 BLOCKER，F1 MAJOR + F2/RC4d-C1 MINOR 测试完备性折叠；636 测试/11 模块；未 push/deploy）→ ✅ **1.10.0 GA 已切版（`v1.10.0`，FF 合并到 master，未推送；GA 就绪审计 gaReady + 2 MAJOR 文档/元数据缺陷折叠）；2026-06-21 已发布到 Maven Central** |
...
| `1.10.0` | **IM 平台基础线**：RC1 ClusterRoomRegistry（per-room 节点定向路由）+ RC2 消息历史/离线队列 + RC3 多设备 presence + RC4 node-to-node mesh（真正的节点天花板突破在此） | **✅ GA 已切版（`v1.10.0`，FF 合并，未推送；644 测试/11 模块——含切版后 JaCoCo 引导的 mesh 覆盖率补强 +8）；2026-06-21 已发布到 Maven Central** |
