...
- `1.1.0-RC2` 已从 P4/P4.1 稳定线切出并发布；`1.2.0` 承接 P5/P5.x WebSocket 产品能力，当前已进入正式版发布收口。
...
- `1.2.0`：当前正式版收口线，承接 P5 首批 WebSocket 产品能力和 P5.x 高级能力。
...
3. 当前主线已收口到 `1.2.0` 发布候选，P5.x 心跳/空闲断线第一刀已完成，WebSocket 应用层消息加密已具备扩展点和内置 AES-GCM 首版实现。
4. P5 基本稳定后进入 `P6`：Micrometer/Actuator 指标、关闭原因维度、日志与运行时诊断增强。

## 企业生产就绪度评估

结论：在“暂时不考虑安全问题”的前提下，`1.1.0-RC2` 已可用于功能/稳定性候选验证；当前主线已把 `1.2.0` 的 P5/P5.x 能力推进到正式版发布检查阶段。
...
- P5.x 第二刀已完成首版：新增默认关闭的 `server.netty.websocket.crypto.*` 配置命名空间、`MessageCryptoCodec` 扩展点、`MessageCryptoKeyProvider` 密钥解析接口和内置 `AES-GCM` 实现；发送侧在写出 frame 前加密，接收侧在 handler 参数绑定前解密，默认拒绝对应 text/binary 类型上的未加密数据帧。
...
- 支持按 URI 或按 session 选择是否加密，避免和公开 echo、健康检查、调试通道混用。
...
- `1.2.0`：当前正式版收口目标，`P5` 首批 WebSocket 产品能力增强已落地，心跳/空闲断线第一刀已完成，应用层消息加密扩展点和 AES-GCM 首版实现已完成。
- `1.2.x`：`P5.x` 继续补齐高级产品能力，优先承接握手鉴权扩展、URI/session 粒度 crypto 策略、密钥轮换示例和浏览器端加密 demo。
- `1.3.0`：`P6`，可观测与运维能力建设。
