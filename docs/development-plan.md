...
- **`1.10.0` GA——已切版 + 打标签（`v1.10.0`，FF 合并到 master，推送+部署待用户执行）（「IM 平台基础」线，Boot 2.7 栈，2.0.0 Boot 3.x 仍独立）**：RC1 `ClusterRoomRegistry`（**per-room 节点定向路由** —— 见下方 ⚠️ 设计修正）+ RC2 消息历史/离线队列 + RC3 多设备 presence + RC4 node-to-node mesh。定位驱动：1.9.0 GA 后做了竞品深挖 + 定位研究（见 `docs/superpowers/notes/`），结论是 IM/实时通信原语 + 突破节点天花板是最高背书方向。
...
## `1.10.0` IM 平台基础线（GA——已切版，推送+部署待执行）
...
| `1.10.0` | **IM 平台基础线**：RC1 ClusterRoomRegistry（per-room 节点定向路由）+ RC2 消息历史/离线队列 + RC3 多设备 presence + RC4 node-to-node mesh（真正的节点天花板突破在此） | **✅ GA 已切版（`v1.10.0`，FF 合并，未推送；644 测试/11 模块——含切版后 JaCoCo 引导的 mesh 覆盖率补强 +8）；推送+部署待用户执行** |
