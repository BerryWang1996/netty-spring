...
| `1.10.0` | **IM 平台基础线**：RC1 ClusterRoomRegistry（per-room 节点定向路由）+ RC2 消息历史/离线队列 + RC3 多设备 presence + RC4 node-to-node mesh（真正的节点天花板突破在此） | **✅ GA 已切版（`v1.10.0`，FF 合并，未推送；636 测试/11 模块）；推送+部署待用户执行** |
