...
- **Integration — single-node Redis Cluster (`ClusterTestRedisCluster` resolver, gated-skip):** the real verification of the cluster client code path. A **single-node** cluster (`redis:7` `--cluster-enabled yes`, then `CLUSTER ADDSLOTSRANGE 0 16383` so the one node owns all 16384 slots) avoids the multi-node announce-ip/gossip problem entirely — with no gossip partners the client connects directly to the mapped port — yet exercises the **real `RedisClusterClient` + `StatefulRedisCluster[PubSub]Connection`** path: cluster pub/sub publish→receive, and slot-routed `HSET`/`HGET`/`SADD`/`SREM`/`DEL`/`SET`/`EXISTS`/`SCAN`. Resolver order mirrors `ClusterTestRedis`: env `CLUSTER_TEST_REDIS_CLUSTER_NODES` → a Testcontainers single-node cluster (started + slots assigned via an exec'd `redis-cli cluster addslotsrange`) → `assumeTrue`-skip. `RedisClusterIntegrationTest` runs all three impls against it: register/lookup/deregister round-trip, a broadcast publish→subscribe, and a heartbeat write/read/reap. **Scope of the proof:** this verifies the cluster *client API correctness* (the main risk) against a genuine cluster; it does NOT cover multi-node slot distribution or cross-node pub/sub propagation (needs a multi-node cluster — out of reach for a flake-free harness, noted in docs).
...
- **New:** `…cluster/redis/RedisClusterPubSubBroker.java`, `RedisClusterSessionRegistry.java`, `RedisClusterNodeHeartbeat.java`; unit tests for each (Mockito over Lettuce cluster command interfaces); a `ClusterTestRedisCluster` test-support resolver (single-node cluster: localhost env → Testcontainers `redis:7` cluster-enabled + `addslotsrange` → skip); `RedisClusterIntegrationTest.java` (runs the three impls against the single-node cluster).
