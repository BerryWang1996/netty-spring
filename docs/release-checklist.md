...
集群扩展项（仍推迟）：NATS broker（ADR-001）、多 pub/sub 连接并行解码、sharded pub/sub、Redis Cluster 客户端一等支持、W3C TraceContext 跨节点传播、可运行的多节点 Docker 示例（Compose + LB + 浏览器）。详见 `development-plan.md`。（已落地并移出本列表：可靠投递 `reliableBroadcast` → RC2；HMAC envelope 认证 → RC3；完整 Micrometer 集群指标 → RC4；多节点 E2E + Testcontainers CI + 跨节点单播 hook-wiring 修复 → RC5。）
